...
The files in this repository provide users with a practical introduction to making senes of data and understand variation using Microsoft Excel. For those new to process behavior charts and understanding variation, it is recommended that users explore the files in this repository in the following sequence:

1. HowToBuildAnXmRChart.xlsx
2. TheRightAndWrongWayToCalculateProcessLimits.xlsx
3. XmRChartSoftwareVerification.xlsx
...
## Files
- **HowToBuildAnXmRChart.xlsx**: This file provides users with step-by-step instructions on how to build an XmR chart in Excel. The XmR chart is the easiests to construct and most useful type of process behavior chart. The fundamentals in this file will make the user aware of the the forumulas used to calculate process limits and the construction of the XmR chart.
- **TheRightAndWrongWayToCalculateProcessLimits.xlsx**: This file provides users with an example of the practical differences that result from calculating process limits the right way, using the 2.660 and 3.268 scaling factors and average moving range, and the wrong way, using three-times the standard deviation. Understanding these differences are essential and should not be overlooked.
- **XmRChartSoftwareVerification.xlsx**: This file provides users with a dataset that can be used to evaluate if software-calculate process limits are calculated correctly, using the 2.660 and 3.268 scaling factors and average moving range. 
